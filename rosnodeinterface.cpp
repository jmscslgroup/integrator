...
// Generated 02-May-2021 21:34:11
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"integrator\" **\n");
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(integrator_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(integrator_M)
                == (NULL)) || rtmGetStopRequested(integrator_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(integrator_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
